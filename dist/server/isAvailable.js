...
function isAvailable(conditionQuery, conditions) {
    ...
    if (conditionQuery["age"] < conditions["JA0110"] ||
    ...
    {
        const myPersonalAttributes = Object.keys(conditionQuery).filter((v) => v.startsWith("JA03"));
        const intersectedAttributes = myPersonalAttributes.filter((a) => conditions[a] === "Y");
    ...
    {
        const myFamilyAttributes = Object.keys(conditionQuery).filter((v) => v.startsWith("JA04"));
        const intersectedAttributes = myFamilyAttributes.filter((a) => conditions[a] === "Y");
        if (intersectedAttributes.length === 0) {
            return false;
        }
    }
